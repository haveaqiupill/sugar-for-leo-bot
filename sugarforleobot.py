...
def start(bot, update):
    ...
    #
    # else:
    #     logger.info("User {} with User ID {} just started conversation with bot.".format(user.username if user.username else user.first_name, user.id))
...
def chatwithparent(bot, update):
    try:
        user = update.message.from_user
        consume_time = html.escape(update.message.text.strip())
        INFOSTORE[user.id]["food"]["consume_time"] = consume_time
        logger.info("User {} has just submitted food consume time".format(user.username if user.username else user.first_name))
        chatid = update.message.chat.id
...
def main():
    ...
    sfl_convo_handler = ConversationHandler(
            entry_points = [CommandHandler('start', start)],
            
            states = {
                AFTER_START: [CallbackQueryHandler(callback = chatwithbaby, pattern = '^(foodrescue)$'),
                                CallbackQueryHandler(callback = chatwithparent, pattern = '^(feedback)$'),
    ...
    dispatcher.add_handler(sfl_convo_handler)
